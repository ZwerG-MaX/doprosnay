...
2. Поднимите стек:
   ...
   ```
3. Откройте **http://pult.local** (или http://localhost):
   - При первом запуске система предложит создать первого администратора
   - После создания администратора вы сможете войти и управлять системой
4. Откройте другие сервисы:
...
- **Первый администратор** — создаётся при первом запуске системы через экран входа.
  Получает полный доступ ко всем функциям системы.
